...
void initSoftAP(){
  ...
  server.addHandler(new CaptiveRequestHandler()).setFilter(
  ...
  server.on("/set_ap", HTTP_POST, [](AsyncWebServerRequest *request){},
    NULL,
  ...
  server.on("/status", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    response["is_working"] = current.appliance_working;
    String responseBody;
    serializeJson(response, responseBody);
    request->send(200, "application/json", responseBody);
  });  
...
private:
    static void IRAM_ATTR CF1Interrupt() {
        // 由于是静态函数，需要通过全局对象访问实例变量
        unsigned long current_time = micros();
        current.pulse_interval = current_time - current.last_pulse_time;
        current.last_pulse_time = current_time;
        current.pulse_count++;
    }
...
public:
    ...
    void begin() {
      ...
      attachInterrupt(digitalPinToInterrupt(CF1_PIN), Current::CF1Interrupt, RISING);
    }
...
void loop(){
  dnsServer.processNextRequest();//强制门户的runtime
  if(wificonfig.config_renewed == true){
    wificonfig.config_renewed = false;
    initSTA(wificonfig.ssid, wificonfig.password);
  }
  if(wificonfig.connStatus==true)ArduinoOTA.handle();//OTA服务runtime

  delay(100);
  current.update();
  Serial.println(current.getStatus());
}
